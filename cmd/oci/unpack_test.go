...
func TestUnpackFreshFixtures(t *testing.T) {
	for _, c := range []struct {
		...
		{name: "hardlink shares the inode", layers: [][]tarEntry{
...
func TestUnpackFreshCleansUpStaging(t *testing.T) {
	for _, c := range []struct {
		name        string
		plant       func(t *testing.T, s *store, dest string)
		wantEntries int
	}{
		{name: "corrupt layer", wantEntries: 0, plant: func(t *testing.T, s *store, dest string) {
			m := manifestOf(t, s, storedDigest(t, s))
		...
		{name: "regular file at dest", wantEntries: 1, plant: func(t *testing.T, s *store, dest string) {
			if err := os.WriteFile(dest, nil, 0o644); err != nil {
				t.Fatal(err)
			}
		}},
		{name: "dangling symlink at dest", wantEntries: 1, plant: func(t *testing.T, s *store, dest string) {
...
// storedDigest returns the single reference pinned in s, so a table row can
// reach the manifest without threading the digest through the case.
func storedDigest(t *testing.T, s *store) string {
	t.Helper()
	d, err := s.digestFor("bad:1", defaultPlatform)
	if err != nil {
		t.Fatal(err)
	}
	return d
...
// docs/usage.md points --rootfs at paths that are symlinks on macOS, so a
// symlinked directory must be followed rather than refused.
...
// unpack must apply the store-format checks pull enforces, and must not create
// a store as a side effect of a lookup.
func TestCmdUnpackChecksStoreFormat(t *testing.T) {
	legacy := t.TempDir()
	if err := os.WriteFile(filepath.Join(legacy, "refs.json"), []byte("{}"), 0o600); err != nil {
		t.Fatal(err)
	}
	_, err := runCaptured(t, "unpack", "--store", legacy, "demo:1")
	if err == nil || !strings.Contains(err.Error(), "legacy refs.json") {
		t.Fatalf("legacy store: err = %v, want the legacy refusal", err)
	}
...
// only at the boundary between layers.
...
func TestUnpackRefusesEscapingPaths(t *testing.T) {
	for _, entries := range [][]tarEntry{
		...
		{{Name: "link", Link: "../../outside"}, {Name: "link/file", Body: "bad"}},
