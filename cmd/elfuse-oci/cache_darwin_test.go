...
func TestDarwinCacheExistsBundleAndPlainRootfs(t *testing.T) {
	...
	mustExists := func(want bool, context string) {
		t.Helper()
		got, err := cacheExists(root, digest)
		if err != nil {
			t.Fatalf("cacheExists (%s): %v", context, err)
		}
		if got != want {
			t.Fatalf("cacheExists = %v for %s, want %v", got, context, want)
		}
	}
	mustExists(false, "absent cache")
	...
	mustExists(true, "sparsebundle cache")
	...
	mustExists(true, "plain rootfs cache")
	if _, err := cacheExists(root, "not-a-digest"); err == nil {
		t.Fatal("cacheExists accepted an invalid digest, want error")
	}
}
...
func TestDarwinPruneCSBundleDryRunDoesNotSweepOrDelete(t *testing.T) {
	...
	// listSweepableClones is only sound while run.lock is held: a clone without
	// a keep marker is abandoned only if no run can be creating one, and a run
	// creates its clone before writing the marker. Probing the lock and
	// releasing it before the listing left a window where a starting run's
	// clone would be reported as reapable. flock conflicts between separate
	// opens of the same file even within one process, so probing from inside
	// the listing window observes whether the dry run really holds it.
	t.Run("run lock held across the listing", func(t *testing.T) {
		bundle := filepath.Join(t.TempDir(), "bundle")
		writeSparseBundleMarker(t, bundle)
		mnt := filepath.Join(bundle, "mnt")
		if err := os.MkdirAll(mnt, 0o755); err != nil {
		...
		heldDuringListing := false
		noDetach(t, func(path string) bool {
			if path == mnt {
				heldDuringListing = csBundleBusy(bundle)
			}
			return path == mnt
		})

		dryRun(t, bundle)
		if !heldDuringListing {
			t.Error("run.lock was free while the dry run listed clones, want it held across the listing")
		}
	})
}
...
// TestDarwinRemoveRefCachesBusyPlainRootfsLeavesBundle: when a digest
