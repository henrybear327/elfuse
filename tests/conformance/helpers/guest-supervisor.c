 ...
 * QEMU_EXEC_SLACK_SEC in tests/conformance/ltp.py so the
 ...
 * 128 plus the signal number, or one of the EXIT_* codes below.
 * Cross-compiled as a static Linux AArch64 binary by the fixture
 * builder; it never builds for the host.
 */
...
int main(int argc, char **argv)
{
    ...
    if (cwd[0] != '/') {
        fprintf(stderr, "guest-supervisor: --cwd must be absolute\n");
        return EXIT_INTERNAL;
    }
    ...
    if (prctl(PR_SET_CHILD_SUBREAPER, 1) < 0) {
        perror("guest-supervisor: prctl");
        return EXIT_INTERNAL;
    }

    if (pipe(error_pipe) < 0) {
        perror("guest-supervisor: pipe");
        return EXIT_INTERNAL;
    }
    if (fcntl(error_pipe[1], F_SETFD, FD_CLOEXEC) < 0) {
        perror("guest-supervisor: fcntl");
        return EXIT_INTERNAL;
    }
