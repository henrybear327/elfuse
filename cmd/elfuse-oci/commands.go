...
func cmdUnpack(args []string) error {
	...
	if rootfs == "" {
		...
		if _, err := storeRootfsPublished(rootfs); err != nil {
			return err
		}
	}
...
func cmdRun(args []string) error {
	...
	if useCS {
		// refLock stays held by this wrapper AND is handed to the guest child
		// (spawnElfuseWait's ExtraFiles), so the reference lock lives for the
		// whole run even when the wrapper dies uncatchably: rmi sees the
		// digest in use before the bundle exists and for as long as elfuse
		// still executes.
		return runCaseSensitive(cf, s, ref, img, digestStr, cfg, rf, tail, refLock)
	}
	...
	if storeManaged {
		...
		rootfsLock = refLock
	} else if insideStore(cf.store, rf.rootfs) {
		// An explicit --rootfs naming the store's own managed trees would
		// run without the per-digest flock, and rmi/prune decide liveness by
		// that flock alone: the sweeps could reclaim the tree under the live
		// guest. Reject it, keeping the user-managed premise above true.
		return fmt.Errorf(
			"--rootfs %s is inside the store; drop --rootfs to run the managed cache",
			rf.rootfs)
	}
	...
	// A store-managed path must also be a real directory, or the run would
	// execute against whatever a planted symlink points at.
	var published bool
	if storeManaged {
		if published, err = storeRootfsPublished(rf.rootfs); err != nil {
			return err
		}
	} else if _, serr := os.Stat(rf.rootfs); serr == nil {
		published = true
