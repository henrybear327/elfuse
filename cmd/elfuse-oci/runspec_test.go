...
// TestComputeRunSpecWorkdirNormalized pins that an absolute WorkingDir is
// cleaned before use: the guest path resolver folds "//" and clamps "/.." at
// the root (src/syscall/path.c), so a config the runtime would accept must
// not fail the pre-launch ensureWorkdir, whose os.Root rejects a raw leading
// separator or "..". The old behavior forwarded the raw string and aborted
// the run before launch.
...
// TestComputeRunSpecRejectsEmptyEnvKey pins that a user --env with an empty
// variable name fails before any rootfs work: elfuse rejects the same input
// (src/main.c), but only after unpack, /etc injection, and workdir creation.
// The old behavior forwarded "=VAL" into that late failure and silently
// dropped "". Image-carried empty keys stay droppable (resolveEnv): such an
// image still starts under Docker.
func TestComputeRunSpecRejectsEmptyEnvKey(t *testing.T) {
	for _, bad := range []string{"=VAL", ""} {
		cfg := &v1.ConfigFile{Config: v1.Config{Cmd: []string{"/hello"}}}
		_, err := computeRunSpec(cfg, runFlags{env: []string{bad}}, t.TempDir(), nil)
		if err == nil || !strings.Contains(err.Error(), "empty variable name") {
			t.Errorf("env %q: err=%v, want an empty-variable-name error", bad, err)
		}
	}
}
...
// TestComputeRunSpecRelativePathEntries pins the POSIX treatment of
// non-absolute PATH elements runc inherits from exec.LookPath: an empty
// element names the working directory and a relative one resolves against
// it, both confined to the rootfs. The old behavior skipped such entries
// unconditionally, failing a bare command Docker starts.
