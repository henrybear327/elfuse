...
# whose argv names the run's own pidfile, as qemu's does. A start that never
# boots has to report what the VM said before its run directory goes. No VM
# boots here: a stub stands in for qemu.
...
# A failed start: the rundir it would explain itself with is removed on the way
# out, so the runner has to have said everything before returning.
stub="$work/qemu-stub"
cat > "$stub" << 'STUB'
#!/usr/bin/env bash
serial=""
