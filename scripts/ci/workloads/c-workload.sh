# shellcheck shell=sh
# C-compile image workload, run in the guest via
...
# than a literal tab, which keeps this heredoc robust. Scaled down from the
# profiled 30-file project + 8 MB TU to keep CI minutes sane; the syscall signature
...
# generic name like /tmp/cwork would read and write whatever the runner left in
# its own /tmp; an elfuse-owned name is created inside the rootfs instead.
d=/tmp/elfuse-c-work
rm -rf "$d"
mkdir -p "$d"
cd "$d"
