...
namespace-wide cursor with directed `kill` resolving ancestors through
the process registry and a guest signal death reported as WIFSIGNALED,
which is what makes LTP's own heartbeat and timeout detection work
across a nested fork.

The divergences that remain, each annotated in `baseline-elfuse.json`:
...
still pass). Its cause is the directory `st_nlink` gap described in the
list below: elfuse reports the host directory link count, which APFS
computes differently from Linux. The fix is a directory-link-count
translation in `translate_stat`, which corrects every sysroot kind.
...
Work the barriers in this order, never editing the manifest or baselines
merely to hide a step:

1. Close the errno-parity gaps in the readv/writev/preadv/pwritev
   ...
   can pass.
2. Provide truthful dynamic process state for the two futex procfs
   probes and a real read-only `pid_max`. Exit criterion: the three
   affected tests record real assertion passes.
3. Translate directory `st_nlink` in `translate_stat` (clears the tmpdir
...
4. After each step, rerun QEMU before elfuse and record; keep any newly
