...
func listSweepableClones(mountPath string) []string {
	...
	for _, e := range entries {
		name := e.Name()
		// Both branches reap staging DIRECTORIES only; a plain file wearing
		// either name did not come from this package and is not swept.
		if !e.IsDir() {
			continue
		}
		switch {
...
// nothing: run.lock is opened without O_CREATE (the bundle twin of
// rootfsCacheBusy), so a missing file means no holder rather than a file
// conjured into every bundle a dry run merely looks at. It deliberately does
// NOT touch attach.lock: attach.lock is a lifecycle lock whose holders
// (provision, sweep, a run's last-one-out Close) each own the mount's detach
// fate, and a read-only prober transiently holding it would fool a
// concurrent Close into skipping its detach (leaving the volume attached
// with no owner). Any failure other than the missing lock file fails closed
// (busy) so a dry-run never advertises a reap it could not safely perform.
func csBundleBusy(bundle string) bool {
	f, err := os.OpenFile(runLockPath(bundle), os.O_RDWR, 0)
	if err != nil {
