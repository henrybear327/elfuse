...
// cacheExists reports whether digest has any unpacked cache under the store:
// the case-sensitive sparsebundle bundle and/or the plain rootfs directory.
// Lstat, not Stat: unpack and run refuse a symlink at the cache path, so a
// dangling link planted there must read as present (and thus removable), not
// vanish behind the follow. A probe that cannot reach a verdict returns its
// error so rmi fails closed instead of dropping the image while silently
// leaving the cache behind.
func cacheExists(root, digest string) (bool, error) {
	bundle, err := csBundleDirForDigest(root, digest)
	if err != nil {
		return false, err
	}
...
// cacheBusyForDigest reports whether a live run holds either of digest's
// unpacked caches: the sparsebundle bundle (run.lock, held from before the
// volume is attached until guest exit) or the plain rootfs directory (its
// sibling per-digest lock). Either one proves the digest is in use. A digest
// with no bundle probes as idle (csBundleBusy treats the missing lock file
// as no-holder), so no existence pre-check is needed.
func cacheBusyForDigest(root, digest string) bool {
	if bundle, err := csBundleDirForDigest(root, digest); err == nil && csBundleBusy(bundle) {
		return true
	}
	rootfs, err := defaultRootfsForDigest(root, digest)
	if err != nil {
...
func pruneCSBundle(rep pruneReport, bundle, key string, live map[string]bool, opts pruneOpts) (pruneReport, error) {
	...
	if opts.dryRun {
		...
		// reap. Read-only: hold the lock and list, never detach or remove.
		//
		// The lock is held across the listing, not merely probed and released,
		// because listSweepableClones treats a clone without a keep marker as
		// abandoned and that is only true while no run can be creating one: a
		// run makes its clone before writing the marker, so a probe-then-list
		// would report a starting run's clone as reapable.
		runLock, err := acquireFlock(runLockPath(bundle), syscall.LOCK_EX|syscall.LOCK_NB)
		if err != nil {
			return rep, nil
		}
		defer runLock.Close()
		mnt := mountPointPath(bundle)
		if isMountPointFn(mnt) {
			rep.CacheDirs = append(rep.CacheDirs, listSweepableClones(mnt)...)
		}
		image := imagePath(bundle)
