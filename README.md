...
filesystem, so a dynamically-linked guest needs no hand-built `--sysroot` tree.
The execution model is the one every elfuse run uses (Linux ELF, elfuse syscall
translation, macOS kernel), with no VM and no Linux kernel in the loop.
