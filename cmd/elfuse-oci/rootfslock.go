...
// sweepOrphanRootfsLock removes dir's sibling lock file when the cache dir
// itself is absent: every run takes the per-digest reference lock and so
// creates the file without ever creating the plain dir, and a failed unpack
// leaves one too. The unlink happens while holding the exclusive lock
// (removeRootfsCache's order), so a live holder keeps its lock and a racing
// acquirer is caught by acquireFlock's stat-after-lock retry.
func sweepOrphanRootfsLock(dir string) error {
	unlock, busy, err := lockRootfsCacheForRemoval(dir)
	if err != nil {
		if os.IsNotExist(err) {
			return nil
