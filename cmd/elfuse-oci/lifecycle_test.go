...
// TestPruneReclaimsRepulledTagPriorImage: re-pulling a mutable tag to
...
// TestCSBundleBusyProbeCreatesNoState pins the probe's read-only contract:
// run.lock is opened without O_CREATE, a missing file means no holder
// (holders create it), and any other failure still fails closed to busy.
// The old probe created run.lock in every bundle a prune --dry-run merely
// looked at, mutating the store the docstring promised to leave alone.
func TestCSBundleBusyProbeCreatesNoState(t *testing.T) {
	bundle := t.TempDir()
	if csBundleBusy(bundle) {
		t.Error("bundle without run.lock reads busy, want idle")
	}
	if _, err := os.Lstat(runLockPath(bundle)); !os.IsNotExist(err) {
		t.Errorf("busy probe created run.lock: %v", err)
	}
}

// TestPruneCacheSweepsOrphanLockFiles pins that prune --cache reclaims a
// sibling <hex>.lock whose cache dir does not exist. Every case-sensitive
// run takes the per-digest reference lock and so creates the lock file
// without ever creating the plain cache dir, and a failed unpack leaves one
// too; the old sweep visited directories only, so these zero-byte orphans
// accumulated without bound. A lock whose holder is live stays.
func TestPruneCacheSweepsOrphanLockFiles(t *testing.T) {
	s := openTestStore(t)
	base := filepath.Join(s.root, rootfsCacheDirName, "sha256")
	if err := os.MkdirAll(base, 0o755); err != nil {
		t.Fatal(err)
	}
	orphan := filepath.Join(base, strings.Repeat("aa", 32)+".lock")
	if err := os.WriteFile(orphan, nil, 0o644); err != nil {
		t.Fatal(err)
	}
	heldDir := filepath.Join(base, strings.Repeat("bb", 32))
	hold, err := acquireRootfsRunLock(heldDir)
	if err != nil {
		t.Fatal(err)
	}
	defer hold.Close()

	if err := cmdPrune([]string{"--store", s.root, "--cache"}); err != nil {
		t.Fatalf("prune --cache: %v", err)
	}
	if _, err := os.Lstat(orphan); !os.IsNotExist(err) {
		t.Errorf("orphan lock survived prune --cache: %v", err)
	}
	if _, err := os.Lstat(rootfsRunLockPath(heldDir)); err != nil {
		t.Errorf("held lock was swept: %v", err)
	}
}

// TestCacheExistsDanglingSymlink pins the Lstat probe: unpack and run refuse
// a symlink at the cache path, so a dangling link planted there must read as
// present (and thus removable by rmi), not vanish behind Stat's follow. The
// old probe followed the link, answered absent forever, and rmi left the
// entry wedged in the store.
func TestCacheExistsDanglingSymlink(t *testing.T) {
	root := t.TempDir()
	...
	if err := os.Symlink(filepath.Join(t.TempDir(), "gone"), rootfs); err != nil {
		t.Fatal(err)
	}
	exists, err := cacheExists(root, digest)
	if err != nil {
		t.Fatal(err)
	}
	if !exists {
		t.Error("dangling symlink at the cache path reads as absent")
	}
}

// TestPruneFailsClosedOnUnresolvablePin pins that gc resolves every liveness
// root before reconciling index.json: a pin whose digest cannot be resolved
// must fail the pass with the descriptors intact. The old order committed
// the descriptor rewrite first, so a malformed pin dropped descriptors it
// could no longer justify, and the pass then failed anyway, permanently
// blocking every later prune and rmi at the same error (the crash window
// rmi documents is a reachable producer of stale pins).
func TestPruneFailsClosedOnUnresolvablePin(t *testing.T) {
	s := openTestStore(t)
	if _, err := s.addImage("local:a", buildImage(t, []string{"/a"})); err != nil {
		t.Fatal(err)
	}
	// Repoint the only pin at a digest NewHash rejects: the reconcile then
	// sees no valid pin covering A's descriptor while reachability errors.
	if err := s.pin("local:a", "sha256:not-hex"); err != nil {
		t.Fatal(err)
	}
	if err := cmdPrune([]string{"--store", s.root}); err == nil {
		t.Fatal("prune with an unresolvable pin succeeded, want error")
	}
	if n := indexManifestCount(t, s.root); n != 1 {
		t.Fatalf("index has %d descriptors after the failed prune, want 1 (drop must not precede validation)", n)
	}
}
...
// TestPruneKeepsBlobsOfBusyUnpinnedDigest: blob reachability must root at the
// per-digest run locks as well as the pin set. resolveImageForUse takes a
// digest's run lock inside the store lock and then releases the store lock,
// so a run keeps reading that manifest's config and layers with no store lock
// held. If a concurrent pull repins the tag meanwhile, the running digest is
// no longer pinned, and a prune rooting only at pins reclaims the very blobs
// that run is still reading: the guest then fails mid-setup with a missing
// blob. The cache sweep already skips busy caches (TestPruneCacheHonorsRoot
// fsRunLock); this pins the same rule for blobs.
func TestPruneKeepsBlobsOfBusyUnpinnedDigest(t *testing.T) {
	s := openTestStore(t)
	imgA := buildImage(t, []string{"/a"})
	imgB := buildImage(t, []string{"/b"})
	...
	if _, err := s.addImage("local:tag", imgB); err != nil { // tag moves A->B
		t.Fatal(err)
	}

	// Stand in for the live run: its cache dir exists and it holds the
	// digest's run lock, exactly as resolveImageForUse leaves things while
	// the run reads A's blobs.
	cache := rootfsForImage(t, s, imgA)
	if err := os.MkdirAll(cache, 0o755); err != nil {
		t.Fatal(err)
	}
	lock, err := acquireRootfsRunLock(cache)
	if err != nil {
		t.Fatal(err)
	}
	defer lock.Close()
	...
	for _, keep := range []v1.Hash{manifestA, configA} {
		if _, err := os.Stat(blobPath(s.root, keep.String())); err != nil {
			t.Errorf("blob %s of the busy digest reclaimed: %v", keep, err)
		}
	}
	if _, err := os.Stat(blobPath(s.root, firstLayerDigest(t, imgA).String())); err != nil {
		t.Errorf("layer blob of the busy digest reclaimed: %v", err)
	}
}
...
func TestListShape(t *testing.T) {
	...
	out := buf.String()
	// CREATED belongs to the human table too: JSON and inspect expose the
	// creation time, and the table dropping a computed column made the two
	// output modes disagree.
	for _, want := range []string{"local:a", "linux/arm64", "CREATED"} {
...
// no run is live. Non-directory entries carrying either name shape stay
// untouched: both branches sweep staging DIRECTORIES only (the old behavior
// swept a plain rootfs.tmp-* file).
func TestListSweepableClonesSkipsKeepMarkerAndNonClones(t *testing.T) {
	...
	tmpFile := filepath.Join(dir, "rootfs.tmp-file")
	runFile := filepath.Join(dir, "run-9999-9")
	for _, p := range []string{tmpFile, runFile} {
	...
	for _, p := range []string{tmpFile, runFile} {
		if _, err := os.Stat(p); err != nil {
			t.Errorf("non-directory %s was reaped, want left alone: %v", p, err)
		}
	}
}
...
// TestListSweepableClonesReapsImageShippedKeepFile: a clone whose
