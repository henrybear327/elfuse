...
Loaded by kirk's plugin discovery (see tests/conformance/kirk_shim.py). Each
...
guest-supervisor (tests/conformance/helpers/) chroots into the copied glibc
...
(QEMU_EXEC_SLACK_SEC in tests/conformance/ltp.py), which stays below
...
from _common import (LAUNCHER, guest_environment, harness_status,
                     rewrite_command)
...
class QemuChrootComChannel(ComChannel):
    ...
    async def _ssh(self, command: str, cap: float) -> Dict[str, Any]:
        ...
        try:
            ...
            raise CommunicationError(
                ...
                harness_status(source="qemu-channel",
                               channel_timed_out=True)
            ) from err
        finally:
        ...
        if proc.returncode == 255:
            raise CommunicationError(
                f"SSH transport to the VM was lost (rc 255): "
                f"{stdout[-400:]}\n" +
                harness_status(source="qemu-channel",
                               execution="transport",
    ...
    async def communicate(self, iobuffer: Optional[IOBuffer] = None) -> None:
        ...
        # qemu-runner mounts /tmp as a tmpfs at the kernel default of
        # half the VM's RAM; the staged sweep rootfs alone is ~1.3 GiB,
        # so grow the mount before copying into it.
        try:
            await self._ssh_check(
                "mount -o remount,size=80% /tmp", AUX_CAP_SEC, "sizing /tmp"
            )
            self._rootfs_ready = True
            await self._ssh_check(
                f"rm -rf {CHROOT} && "
                f"cp -a {shlex.quote(self._rootfs_src)} {CHROOT}",
                SUITE_SETUP_CAP_SEC,
                "copying the rootfs into the VM",
            )
            for prefix, target in MOUNTS:
                await self._ssh_check(
                    f"{prefix} {CHROOT}{target}", AUX_CAP_SEC,
                    f"mounting {target}"
                )
            ...
            try:
                ...
                self._logger.warning("setup rollback incomplete: %s", err)
            raise
        self._active = True
    ...
    async def _teardown(self) -> None:
        ...
        for target in reversed(list(self._mounted)):
            try:
                await self._ssh_check(
                    f"umount {CHROOT}{target}", AUX_CAP_SEC,
                    f"unmounting {target}"
                )
            except CommunicationError as err:
        ...
        if failed:
            raise CommunicationError("; ".join(failed))
        if self._rootfs_ready:
            await self._ssh_check(f"rm -rf {CHROOT}", SUITE_SETUP_CAP_SEC,
                                  "removing the copied rootfs")
            self._rootfs_ready = False
    ...
    async def _status_note(self, status_path: str) -> str:
        """Append human context and the authoritative machine record."""
        ret = await self._ssh(f"cat {status_path}; rm -f {status_path}", AUX_CAP_SEC)
        if ret["returncode"] != 0:
            return ("ltp-harness: supervisor status file missing\n" +
                    harness_status(source="qemu-supervisor",
                                   error="supervisor status file missing"))
        ...
        try:
            ...
            return ("ltp-harness: supervisor status file malformed\n" +
                    harness_status(source="qemu-supervisor",
                                   error="supervisor status file malformed"))
        if not isinstance(data, dict):
            return ("ltp-harness: supervisor status file malformed\n" +
                    harness_status(source="qemu-supervisor",
                                   error="supervisor status is not an object"))
        ...
        human = ""
        if notes:
            human = "ltp-harness: " + "; ".join(notes) + "\n"
        data["source"] = "qemu-supervisor"
        return human + harness_status(**data)
