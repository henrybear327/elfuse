...
func TestCmdPullPinsImageOffline(t *testing.T) {
	...
	if gotRef != "local:tiny" || gotOptions != 2 {
		t.Fatalf("fake crane.Pull got ref=%q options=%d, want local:tiny with the platform and keychain options", gotRef, gotOptions)
	}
...
// TestCmdRunRejectsExplicitRootfsInsideStore pins that --rootfs naming the
// store's own managed trees (the rootfs/ digest caches, the cs/ bundle dirs)
// is refused. The explicit-rootfs path runs without the per-digest flock and
// prune/rmi decide liveness by that flock alone, so the old behavior let the
// sweeps reclaim the cache under the live guest.
func TestCmdRunRejectsExplicitRootfsInsideStore(t *testing.T) {
	s := openTestStore(t)
	digest, err := s.addImage("local:a", buildImage(t, []string{"/image-cmd"}))
	if err != nil {
		t.Fatal(err)
	}
	...
	execCalled := false
	withFakeExecElfuse(t, func(_ string, _ *runSpec, _ *flockFile) error {
		execCalled = true
		return nil
	})

	_, _, err = captureOutput(t, func() error {
		return cmdRun([]string{"--store", s.root, "--plain-rootfs", "--rootfs", cache, "local:a"})
	})
	if err == nil || !strings.Contains(err.Error(), "inside the store") {
		t.Fatalf("err=%v, want an inside-the-store rejection", err)
	}
	if execCalled {
		t.Fatal("guest exec reached despite the rejected --rootfs")
	}
}
...
// not poison digest A's cache with image B's content. The afterImageResolve
...
// TestRmiRefusesWhileReferenceLockHeld: a starting run holds the
