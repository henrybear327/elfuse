...
func computeRunSpec(cfg *v1.ConfigFile, rf runFlags, rootfs string, tail []string) (*runSpec, error) {
	...
	// --env overrides are user input: reject an empty variable name ("=VAL"
	// or "") before any rootfs work. elfuse applies the same rule
	// (src/main.c), but only after the rootfs has been unpacked and
	// prepared. Image-carried entries stay droppable in resolveEnv: an image
	// shipping one still starts under Docker.
	for _, e := range rf.env {
		if k, _, _ := strings.Cut(e, "="); k == "" {
			return nil, fmt.Errorf("invalid --env %q: empty variable name", e)
		}
	}
	...
	// resolver's clamp (src/syscall/path.c), so a WorkingDir the runtime
	// would accept cannot fail the pre-launch ensureWorkdir: os.Root rejects
	// a raw leading separator or ".." as an escape.
...
// match must be a regular file with an execute bit. Non-absolute PATH
// elements follow the POSIX rule runc inherits from exec.LookPath: an empty
