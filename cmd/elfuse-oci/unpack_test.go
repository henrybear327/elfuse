...
//
// syscall.Umask is process-global: neither this test nor any sibling in the
// package may adopt t.Parallel while the mutation exists.
...
func TestUnpackOpaqueClearsDirectory(t *testing.T) {
	root, dir := newRoot(t)
	// Lower layer ships opt with two files.
	applyEntries(t, root, []tar.Header{
		dirHeader("opt", 0o755),
		regHeader("opt/lower-a", 0o644, 0),
		regHeader("opt/lower-b", 0o644, 0),
	})
	// Upper layer: the opaque marker clears the lower content, then this
	// layer re-adds only lower-a. In one layer the marker would keep both
	// files as same-layer children; the boundary is what makes lower-b go.
	applyEntries(t, root, []tar.Header{
