...
// non-Darwin this is just the plain rootfs directory. Lstat, not Stat: unpack
// and run refuse a symlink at the cache path, so a dangling link planted there
// must read as present (and thus removable), not vanish behind the follow. A
// probe that cannot reach a verdict returns its error so rmi fails closed
// instead of dropping the image while silently leaving the cache behind.
...
// cacheBusyForDigest reports whether a live run holds digest's unpacked cache.
// On non-Darwin the only cache is the plain rootfs directory, guarded by its
// sibling per-digest lock.
