...
// elfuse-oci is the OCI image CLI for elfuse. It owns the image pipeline
// using go-containerregistry, so elfuse itself stays a Linux
...
// flags themselves: the registrations are the single source, and a
// hand-maintained flag list cannot drift from what parsing accepts.
type command struct {
	name    string
	args    string
	summary string
