...
func inspect(w io.Writer, s *store, ref string, platform Platform, asJSON bool) error {
	img, err := s.image(ref, platform)
	if err != nil {
		return err
	}
	...
	if asJSON {
		...
		// should diff cleanly against `skopeo inspect --config`. The summary
		// fields below are never printed, so their blob reads are skipped.
