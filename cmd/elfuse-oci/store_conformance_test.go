...
// TestWriteFileDurableLeavesNoTempOnRenameFailure drives the staging-cleanup
// branch the read-only-dir case cannot reach (there CreateTemp fails before
// any temp exists): a non-empty directory at the destination name makes the
// final rename fail after the temp was fully written, so the deferred
// os.Remove must reclaim it. Regression guard for that cleanup: deleting the
// defer in writeFileDurable turns this red.
