...
#
# The build script and the runner both need the same three facts: which
# gVisor commit the payload is pinned to, which targets make up the suite,
# and what a usable artifact looks like (a static AArch64 ELF with no
# interpreter and no shared-library dependencies, because the guest has no
# loader). They live here once so a pin bump cannot move for one consumer
# and not the other.
