...
func captureOutput(t *testing.T, fn func() error) (string, string, error) {
	...
	// Close every pipe end in the defer, not only inline: a fn that panics
	// or runs t.Fatal (Goexit) leaves the write ends open, parking both
	// ReadAll goroutines forever. The double Close is a harmless ErrClosed.
	defer func() {
...
// produce the same layer blob but distinct config/manifest digests: two
// pinned refs sharing one layer, with distinct manifests.
...
// seedImage adds img to s under ref at the default platform (matching
// buildImage's linux/arm64 config), failing the test on error, and returns
// the pinned manifest digest.
func seedImage(t *testing.T, s *store, ref string, img v1.Image) string {
	t.Helper()
	digest, err := s.addImage(ref, defaultPlatform, img)
	if err != nil {
		t.Fatal(err)
	}
	return digest
...
// fakeDigest returns a syntactically valid sha256 digest built from one
// repeated hex nibble, for tests that need distinct digests without hashing
// anything.
