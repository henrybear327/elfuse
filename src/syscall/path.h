...
/* Flags an open(2) of a translated /dev/shm leaf needs: O_NOFOLLOW stops a
 * symlink under the synthetic /dev/shm dir from resolving the leaf out of the
 * namespace, and O_NONBLOCK stops a FIFO leaf from parking the vCPU thread on
 * an open with no writer. Every open of a shm leaf goes through here, so the
 * two requirements cannot be taken separately.
 */
 ...
 * indices. The only sanctioned way to undo that leg; prefix arithmetic on
 * host paths leaks token spellings to the guest.
 *
 * This inverts the sysroot leg only. path_translate_at() can also redirect
 * /dev/shm into its per-UID backing dir and materialize a FUSE object, and
 * neither of those is undone here: a host path produced by one of those legs
 * comes back unchanged. Callers publishing such a path to the guest must
 * carry the guest spelling forward themselves rather than reverse it.
 */
