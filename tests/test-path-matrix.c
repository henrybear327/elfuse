 ...
 *
 * Sectioned coverage of the path-taking syscalls against guest-created
...
static void section_reverse(void)
{
    ...
    EXPECT_TRUE(dir_has_entry("/", "Sym.Link") && dir_has_entry("/", "DirA"),
                "root listing exact names");
    ...
    /* The reverse map copies each component into a NAME_MAX+1 buffer and
     * reassembles the guest path with bounded appends. A maximum-length
     * component exercises both bounds at once: one byte more in either check
     * and this round-trip fails or truncates.
     */
    TEST("NAME_MAX component round-trip");
    char longname[NAME_MAX + 2];
    longname[0] = 'L';
    memset(longname + 1, 'n', NAME_MAX - 1);
    longname[NAME_MAX] = '\0';
    char longdir[NAME_MAX + 8];
    snprintf(longdir, sizeof(longdir), "/%s", longname);
    char lcwd[NAME_MAX + 64] = {0};
    bool lok = (mkdir(longdir, 0755) == 0 || errno == EEXIST) &&
               chdir(longdir) == 0 && getcwd(lcwd, sizeof(lcwd)) != NULL &&
               strcmp(lcwd, longdir) == 0 && strstr(lcwd, ".ef_") == NULL;
    EXPECT_TRUE(lok, "max-length component survives the reverse map");
    chdir("/");

    /* Root itself: the walk yields no components, so the assembled guest path
     * must still be "/" rather than an empty string.
     */
    TEST("sysroot root reverse-maps to /");
    char rcwd[64] = {0};
    EXPECT_TRUE(chdir("/") == 0 && getcwd(rcwd, sizeof(rcwd)) != NULL &&
                    strcmp(rcwd, "/") == 0,
                "root cwd is /");
}
...
static void section_sockets(void)
{
    ...
    /* The sidecar's bookkeeping names are invisible to stat and uncreatable
     * through open(O_CREAT) (section_forward); bind must refuse them on the
     * same terms. A bind whose path carries a '/' resolves the parent in
     * lookup mode and reattaches the leaf verbatim, so the leaf never reaches
     * the create-mode reserved-name guard: unguarded, the index spelling
     * collides with the real bookkeeping file and reports EADDRINUSE, leaking
     * the existence of a name stat() calls ENOENT.
     */
    if (mode_ci) {
        TEST("bind reserved index name");
        int rfd = socket(AF_UNIX, SOCK_STREAM, 0);
        struct sockaddr_un run = {0};
        run.sun_family = AF_UNIX;
        strcpy(run.sun_path, DIR_A "/.elfuse_case_index");
        EXPECT_ERRNO(bind(rfd, (struct sockaddr *) &run, sizeof(run)), ENOENT,
                     "index name must be unbindable");
        if (rfd >= 0)
            close(rfd);

        /* Control: the leafless spelling already translates in create mode and
         * so is guarded today. It pins both spellings to the same errno.
         */
        TEST("bind reserved name at root");
        int qfd = socket(AF_UNIX, SOCK_STREAM, 0);
        struct sockaddr_un qun = {0};
        qun.sun_family = AF_UNIX;
        strcpy(qun.sun_path, "/.elfuse_case_index");
        EXPECT_ERRNO(bind(qfd, (struct sockaddr *) &qun, sizeof(qun)), ENOENT,
...
static void section_inotify(void)
{
    ...
    /* A token that the index does map must never surface under its on-disk
     * spelling: the snapshot reverse-maps it, so the event carries the guest
     * name. Create another mixed-case entry and confirm the event is the guest
     * spelling with no token anywhere in the stream. (A token the index does
     * not map is a different case, covered by test-sidecar-orphan: it keeps its
     * spelling, because that spelling is what a lookup resolves.)
     */
    ...
    /* Scale: the snapshot loads the directory index once and looks each entry
     * up against it, so a directory full of tokens is where a stale or
     * misindexed row would show up. Populate one, then create a single new
     * entry and require exactly one IN_CREATE naming it. A per-entry lookup
     * would report the same thing, so this is a guard on the batched form
     * rather than a new behaviour.
     */
    TEST("inotify scale: one event among many tokens");
    bool scale_ok = false;
    if (ifd >= 0) {
        if (mkdir("/Many.Dir", 0755) != 0 && errno != EEXIST)
            goto scale_done;
        for (int i = 0; i < 64; i++) {
            char p[64];
            snprintf(p, sizeof(p), "/Many.Dir/Ent.%02d", i);
            if (write_file(p, "x\n") != 0)
                goto scale_done;
        }
        /* One entry at the component-length bound, so the batched lookup is
         * exercised at the same edge as the reverse-map bounds test.
         */
        char maxname[NAME_MAX + 16];
        int pre = snprintf(maxname, sizeof(maxname), "/Many.Dir/M");
        memset(maxname + pre, 'x', (size_t) (NAME_MAX - 1));
        maxname[pre + NAME_MAX - 1] = '\0';
        if (write_file(maxname, "m\n") != 0)
            goto scale_done;

        int swd = inotify_add_watch(ifd, "/Many.Dir", IN_CREATE);
        if (swd < 0)
            goto scale_done;
        if (write_file("/Many.Dir/New.Entry", "n\n") != 0)
            goto scale_done;
        ...
        for (int i = 0; i < 50 && !seen; i++) {
            char evb[4096];
            ssize_t rn = read(ifd, evb, sizeof(evb));
            for (ssize_t off = 0; off < rn;) {
                struct inotify_event *ev = (struct inotify_event *) (evb + off);
                if (ev->len) {
                    if (!strcmp(ev->name, "New.Entry"))
                        seen++;
                    else if (!strncmp(ev->name, ".ef_", 4))
                        other = true;
                }
                off += (ssize_t) sizeof(*ev) + ev->len;
            }
        ...
        scale_ok = seen == 1 && !other;
    }
scale_done:
    EXPECT_TRUE(scale_ok, "exactly one guest-named event in a tokenized dir");
...
static void section_private_tmp(void)
{
    ...
    EXPECT_TRUE(dir_has_entry("/tmp", "Prefix.Probe"),
                "tmp listing includes create");
                                   ...
                                   const char *tmp_shadow)
{
    ...
    /* The same normalized-spelling guarantee as /tmp above, for the other two
     * private prefixes: one classifier decides both, so each prefix must
     * survive ".", "..", and doubled separators.
     */
    TEST("/var/tmp non-canonical round-trip");
    bool vnc = vt && stat("//var/tmp/V.Probe", &st) == 0 &&
               stat("/var/./tmp/V.Probe", &st) == 0 &&
               stat("/var/../var/tmp/V.Probe", &st) == 0;
    EXPECT_TRUE(vnc, "normalized spellings reach the private file");
    ...
    TEST(".ccache non-canonical round-trip");
    bool cnc = cc && stat("/Cache.Proj/.ccache/../.ccache/Obj", &st) == 0 &&
               stat("/Cache.Proj/./.ccache/Obj", &st) == 0;
    EXPECT_TRUE(cnc, "normalized spellings reach the private .ccache child");

    /* The escape direction: ".." that walks back out of a private prefix must
     * stop being private, so a normalizer that collapsed ".." too late to
     * matter would show up here rather than as a silent host fall-through.
     */
    TEST("dotdot out of /var/tmp is not private");
    EXPECT_TRUE(stat("/var/tmp/../../etc/hosts", &st) != 0 ||
                    stat("/etc/hosts", &st) == 0,
                "escaping spelling must not be forced private");
}
...
static void section_exec(const char *self)
{
    ...
    bool copied = copy_file_exec(self, DIR_A "/Exec.Copy") == 0;
