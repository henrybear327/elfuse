...
// TestUnpackOpaqueAfterImplicitParentChildren: a late opaque marker
...
// TestUnpackRejectsDotWhiteout pins that a whiteout whose target collapses to
// a dot name fails extraction. A whiteout removes exactly one named sibling,
// but ".wh.." leaves "." and ".wh..." leaves "..", which Join folds into the
// containing directory and its parent.
//
// The ".wh.." lane covers a real defect: the entry was applied and
// RemoveAll(Join("opt", ".")) deleted the whole directory the marker sits in,
// silently discarding every lower layer's content under it. The ".wh..." lane
// is a regression guard: Join yields the rootfs root, which os.Root already
// refuses to remove, so it fails closed today: the subtest pins that the
// rejection stays explicit rather than resting on that accident. Layer
// tarballs are untrusted input, so both must be refused by name.
func TestUnpackRejectsDotWhiteout(t *testing.T) {
	for _, name := range []string{"opt/.wh..", "opt/.wh..."} {
		t.Run(name, func(t *testing.T) {
			...
			if err := applyEntry(root, &hdr, strings.NewReader(""), newLayerPaths()); err == nil {
				t.Errorf("%s entry applied, want invalid-whiteout error", name)
			}
			if _, err := os.Stat(filepath.Join(dest, "opt", "keep")); err != nil {
				t.Errorf("opt/keep after rejected whiteout: %v, want untouched", err)
			}
		})
	}
}
...
// TestUnpackWhiteoutThroughSymlinkKeepsTarget pins that a plain whiteout whose
// parent is a lower layer's symlink does not delete through the link: the
// whiteout hides lower content at its literal path, and a parent that is not a
// real directory means that path does not exist, so the whiteout is a no-op.
// The regression this guards: root.RemoveAll resolves in-root symlink parents,
// so d/.wh.keep over a lower `d -> /target` deleted /target/keep, a file the
// layer never named.
func TestUnpackWhiteoutThroughSymlinkKeepsTarget(t *testing.T) {
	lower := testTarLayer(t,
		tarEntry{header: dirHeader("target", 0o755)},
		tarEntry{header: regHeader("target/keep", 0o644, 0), body: "keep"},
		tarEntry{header: symHeader("d", "/target")},
	...
	upper := testTarLayer(t,
		tarEntry{header: tar.Header{Name: "d/.wh.keep", Typeflag: tar.TypeReg, Mode: 0o644}},
	)

	s := openTestStore(t)
	if _, err := s.addImage("local:whiteout-link", testImageWithLayers(t, lower, upper)); err != nil {
		t.Fatal(err)
	}
	dest := t.TempDir()
	if err := unpackRef(t, s, "local:whiteout-link", dest); err != nil {
		t.Fatalf("unpackImage: %v", err)
	}

	if b, err := os.ReadFile(filepath.Join(dest, "target", "keep")); err != nil || string(b) != "keep" {
		t.Fatalf("target/keep = %q, err=%v; want untouched by whiteout-through-symlink", b, err)
	}
	fi, err := os.Lstat(filepath.Join(dest, "d"))
	if err != nil {
		t.Fatal(err)
	...
	if fi.Mode()&os.ModeSymlink == 0 {
		t.Fatalf("d mode = %v, want the lower symlink left in place (the layer ships no d entry)", fi.Mode())
	}
}

// TestUnpackFailsOnCorruptGzipTrailer pins that a layer blob whose gzip
// trailer (CRC/ISIZE) is corrupted fails the unpack. Tar's end-of-archive
// marker arrives before the compressed stream's end, so without an explicit
// drain the decompressor never reads the trailer and the corruption vanished
// in the deferred Close, whose error is discarded: the old behavior unpacked
// such a layer successfully.
func TestUnpackFailsOnCorruptGzipTrailer(t *testing.T) {
	layer := testTarLayer(t,
		tarEntry{header: dirHeader("etc", 0o755)},
		tarEntry{header: regHeader("etc/keep", 0o644, 0), body: "keep"},
	)
	s := openTestStore(t)
	if _, err := s.addImage("local:trailer", testImageWithLayers(t, layer)); err != nil {
		t.Fatal(err)
	}
	img, err := s.image("local:trailer")
	if err != nil {
		t.Fatal(err)
	}
	layers, err := img.Layers()
	if err != nil || len(layers) != 1 {
		t.Fatalf("layers = %d, err=%v; want 1", len(layers), err)
	}
	digest, err := layers[0].Digest()
	if err != nil {
		t.Fatal(err)
	}
	blob := filepath.Join(s.root, "blobs", digest.Algorithm, digest.Hex)
	raw, err := os.ReadFile(blob)
	if err != nil {
		t.Fatal(err)
	}
	// The gzip member ends with CRC32 (4 bytes) then ISIZE (4 bytes); flip a
	// CRC bit so the deflate payload (and the tar inside it) stays intact.
	raw[len(raw)-8] ^= 0x01
	if err := os.WriteFile(blob, raw, 0o644); err != nil {
		t.Fatal(err)
	}

	if err := unpackRef(t, s, "local:trailer", t.TempDir()); err == nil {
		t.Fatal("unpackImage succeeded on a corrupted gzip trailer, want error")
	}
}

// TestUnpackRefusesSymlinkedStoreRootfs pins that a store-managed rootfs cache
// path which is not a real directory is refused. The store names that path by
// digest and only ever creates it by renaming a staging directory into place,
// so a symlink found there did not come from this package. os.OpenRoot follows
// symlinks in the directory name it is given, so a symlink planted at the
// digest path redirected the whole extraction (and any guest later run against
// it) outside the store. An explicit --rootfs keeps following links, which is
// why the check guards the store-managed path only.
func TestUnpackRefusesSymlinkedStoreRootfs(t *testing.T) {
	s := openTestStore(t)
	digest, err := s.addImage("local:tiny", tinyImage(t))
	if err != nil {
		t.Fatal(err)
	}
	cache, err := defaultRootfsForDigest(s.root, digest)
	if err != nil {
		t.Fatal(err)
	}
	outside := t.TempDir()
	...
	if err == nil {
		t.Error("cmdUnpack into a symlinked store rootfs succeeded, want refusal")
	}
	if entries, rerr := os.ReadDir(outside); rerr != nil || len(entries) != 0 {
		t.Errorf("symlink target holds %d entries (err=%v), want the unpack to never reach it", len(entries), rerr)
	}
}
