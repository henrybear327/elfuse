...
Scenario: the classifier in tests/lib/gvisor_conformance.py is fed the
output shapes a real run produces, including the damaged ones (truncated
and malformed XML, a suite that exits non-zero with a clean XML, log bytes
that are not valid UTF-8, a manifest row naming an unknown case).

Contract pinned: a damaged or missing result is BROKEN and fatal, never a
pass; the expectations manifest must cover the discovered case set exactly;
and an XFAIL that starts passing is itself fatal rather than a silent
improvement.

Failure mode a regression produces: the conformance gate reports green for
a lane that crashed, timed out, or never ran the cases at all, which is the
failure the whole lane is built to make impossible.
"""
