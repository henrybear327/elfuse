...
# When a pinned tag moves, the re-pin strands the old digest's blobs AND its
# unpacked caches; --cache reclaims both so a persistent store stays bounded
# (the stranded sparsebundles dwarf the blobs). Safe beside concurrent legs:
# the sweep runs under the store lock, skips still-pinned digests, and skips
# busy caches via their non-blocking flocks.
