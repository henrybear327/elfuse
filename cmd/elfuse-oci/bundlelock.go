...
// The rest of a bundle directory's layout, named here beside the lock paths so
// the whole shape is defined in one place: hdiutil attaches the volume at the
// mount point, and the sparsebundle is the image file it attaches. Both are
// read by provisioning, the run path, and the prune sweeps.
func mountPointPath(bundle string) string { return filepath.Join(bundle, "mnt") }
func imagePath(bundle string) string      { return filepath.Join(bundle, "rootfs.sparsebundle") }
