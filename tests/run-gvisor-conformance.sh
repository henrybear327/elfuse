...
#
# The lane answers one question: does elfuse resolve each pinned gVisor
# syscall case the same way a real Linux kernel does. Both backends are run
# and every case is classified against tests/conformance/gvisor/expectations.tsv,
# which must cover the discovered set exactly. A case that fails to build,
# times out, dies on a signal, or leaves unparseable output is BROKEN and
# fatal, never a pass, so a lane that reports success has actually run.
