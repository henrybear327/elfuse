 ...
 * variant. path_translation_oflags() supplies the namespace's own requirement
 * (O_NOFOLLOW to keep a symlink leaf contained, O_NONBLOCK so a FIFO leaf
 * cannot block the vCPU thread) from one place, so the policy cannot drift
 * between here and the other translated opens; O_CLOEXEC covers the
 * short-lived fd. See dev_shm_resolve_path().
 */
static int shm_open_leaf(const path_translation_t *tx, int oflags)
{
    return open(tx->host_path, path_translation_oflags(tx, oflags) | O_CLOEXEC);
}
...
/* Resolve dirfd for an AT_EMPTY_PATH mutation and reject the identities that
 * cannot be mutated through one. On success the caller owns er->ref and must
 * close it; on failure the descriptor is already released. Shared by fchmodat
 * and fchownat so the policy cannot drift between them. Stat-like callers do
 * not use this: they serve the FUSE and proc identities instead of refusing
 * them, so they read path_resolve_empty_at directly.
 */
static int64_t empty_at_mutable(guest_fd_t dirfd, path_empty_at_t *er)
{
                     ...
                     int flags)
{
    ...
    if ((flags & LINUX_AT_EMPTY_PATH) && path[0] == '\0') {
        path_empty_at_t er;
        int64_t erc = empty_at_mutable(dirfd, &er);
        if (erc < 0)
            return erc;
