...
.PHONY: test-hello test-all check check-syscall-coverage test-gdbstub test-coreutils test-busybox \
        ...
        test-execveat-shm test-absock-cleanup test-sidecar-orphan \
        test-inotify-index-fail \
...
check: $(ELFUSE_BIN) $(TEST_DEPS) check-syscall-coverage \
	...
	@$(MAKE) --no-print-directory test-sidecar-orphan
	@printf "\n$(BLUE)━━━ inotify index-failure containment ━━━$(RESET)\n"
	@$(MAKE) --no-print-directory test-inotify-index-fail
...
## Path-translation matrix: one sectioned guest binary covering the
...
## A directory index that stops being readable between a watch's baseline and
## its next snapshot must fail the snapshot, not silently drop every mapped
## child, which the diff would then report as deletions of files that still
## exist. The failure has to start after the baseline, so the guest announces
## WATCH_READY and blocks on a host-staged sentinel while this recipe makes the
## index unreadable and provokes the next snapshot.
test-inotify-index-fail: $(ELFUSE_BIN) $(BUILD_DIR)/test-inotify-index-fail
	@set -e; \
	tmpdir=$$(mktemp -d); \
	out=$$(mktemp); \
	trap 'rm -rf "$$tmpdir" "$$out"' EXIT; \
	printf "  %-30s " "index failure spares baseline"; \
	$(ELFUSE_BIN) --sysroot "$$tmpdir" \
	    $(BUILD_DIR)/test-inotify-index-fail > "$$out" 2>/dev/null & \
	guest=$$!; \
	ready=0; \
	for i in $$(seq 1 200); do \
		if grep -q WATCH_READY "$$out" 2>/dev/null; then ready=1; break; fi; \
		sleep 0.1; \
	done; \
	if [ "$$ready" = 0 ]; then \
		kill "$$guest" 2>/dev/null || true; \
		printf "FAIL: guest never armed the watch\n"; \
		exit 1; \
	fi; \
	idx=$$(find "$$tmpdir" -path '*/.ef_*/.elfuse_case_index' -print -quit); \
	if [ -z "$$idx" ]; then \
		kill "$$guest" 2>/dev/null || true; \
		printf "SKIP (no tokenized index; volume is case-sensitive)\n"; \
		exit 0; \
	fi; \
	chmod 000 "$$idx"; \
	: > "$$(dirname "$$idx")/HostNew"; \
	...
	rc=0; wait "$$guest" || rc=$$?; \
	chmod 644 "$$idx"; \
	verdict=$$(sed -n 's/^INDEX_FAIL=//p' "$$out"); \
	if [ "$$verdict" = ok ]; then \
		printf "OK\n"; \
	else \
		printf "FAIL: %s\n" "$${verdict:-unreported}"; \
...
## A sidecar token with no index row is host-private and unreachable by name,
## so a listing must hide it. The orphan token is staged from here because the
## guest cannot create one: every guest-created name is registered in the index
## as it is written. The ordinary sibling staged beside it must still be listed,
## which is what separates hiding an unmapped token from hiding everything the
## index does not name.
test-sidecar-orphan: $(ELFUSE_BIN) $(BUILD_DIR)/test-sidecar-orphan
	@set -e; \
	tmpdir=$$(mktemp -d); \
	trap 'rm -rf "$$tmpdir"' EXIT; \
	mkdir -p "$$tmpdir/data"; \
	: > "$$tmpdir/data/.ef_0123456789abcdef"; \
	printf 'host\n' > "$$tmpdir/data/Plain.Host"; \
	$(ELFUSE_BIN) --sysroot "$$tmpdir" $(BUILD_DIR)/test-sidecar-orphan
