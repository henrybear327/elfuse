...
func captureOutput(t *testing.T, fn func() error) (string, string, error) {
	...
	// or runs t.Fatal (Goexit) would otherwise leave the write ends open,
	// parking both ReadAll goroutines forever and leaking four fds per
	// aborted test. The inline closes below stay because they are what
	// terminates the readers on the success path; the double Close is a
	// harmless ErrClosed.
	defer func() {
