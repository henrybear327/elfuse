...
// timedKeychain runs an inner keychain's Resolve under a deadline. On timeout it
// returns an actionable error instead of blocking forever; the inner goroutine
// is abandoned and reaped when the process exits. It preserves the inner
// keychain's result in the normal case, so registry auth is unchanged when the
// helper answers promptly.
