...
func list(w io.Writer, s *store, asJSON bool) error {
	...
	for _, ref := range refs {
		...
		e.Platform = platformOf(cfg).String()
		if !cfg.Created.IsZero() {
			e.Created = formatCreated(cfg)
		}
	...
	// A failed write (closed pipe, full disk behind a redirect) must not
	// exit 0 with truncated output a script would consume as complete;
	// same contract as inspect.
	if asJSON {
		...
		if _, err := fmt.Fprintf(w, "%s\n", b); err != nil {
			return fmt.Errorf("list: write: %w", err)
		}
		return nil
	}
	if len(entries) == 0 {
	...
	fmt.Fprintf(bw, "%-40s  %-12s  %-14s  %-20s  %12s  %s\n", "REF", "DIGEST", "PLATFORM", "CREATED", "SIZE", "LAYERS")
	for _, e := range entries {
		created := e.Created
		if created == "" {
			created = "-"
		}
		fmt.Fprintf(bw, "%-40s  %s  %-14s  %-20s  %12d  %d\n", e.Ref, shortDigest(e.Digest), e.Platform, created, e.Size, e.Layers)
	}
