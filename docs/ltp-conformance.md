...
```
tests/ltp/manifest.json ────────┐   (curated tiers, hand-written)
tests/ltp/manifest-sweep.json ──┤   (sweep tiers, generated; see below)
tests/ltp/sweep-overrides.json ─┤
tests/ltp/pin.json ─────────────┤
                                v
        harness.py build-fixture          (ltp_harness/fixture.py)
                         │  downloads + verifies LTP and kirk archives,
                         │  cross-builds the payload, stages the
...
externals/test-fixtures/ltp-aarch64/{rootfs,kirk,bin,...}
   ...
   baseline-<backend>[-sweep].json ──> REGRESSION / IMPROVED / green
                         │
                         v                (report.py, passrate.py)
   gate-<backend>.json + results-<backend>.xml
   passrate-<backend>.json + passrate-<backend>.md
```
...
## Selection: curated tiers and the sweep

The lane runs two kinds of selection, both committed:

- The **curated tiers** (`fast`, `extended`, `nightly`) are hand-picked
  entries in `tests/ltp/manifest.json` with individual timeouts and
  notes. They are small, quick, and the ones a developer runs while
  iterating.
- The **sweep tiers** (`sweep`, `sweep-slow`) cover every remaining
  entry of upstream's `runtest/syscalls` file at the pinned release:
  the full suite is exactly curated plus sweep. Their manifest,
  `tests/ltp/manifest-sweep.json`, is generated by
  `harness.py gen-sweep` (`ltp_harness/sweep.py`) and committed, so the
  recorded baselines always gate a reviewable selection and a pin bump
  produces a reviewable selection diff.

The sweep exists because the selectable universe is knowable and large:
`runtest/syscalls` defines 1506 entries at the pinned release, and
elfuse's dispatch table (`src/syscall/dispatch.tbl`) implements 224
syscalls, 192 of which have LTP test directories. Sampling that space
by hand grows the manifest one bug-report at a time; running all of it
measures elfuse against the whole surface, including the syscalls it
does not implement yet. A test elfuse cannot run is a legitimate,
recorded BROKEN baseline entry, not a reason to exclude it: the gate
model already treats a recorded failure as a green state, so breadth
costs runtime, never signal. Pass-rate reporting (below) turns that
breadth into a ranked list of the weakest syscall families.

The generator resolves each runtest entry to its source file to decide
two fields the runtest line does not carry:

- `result_format`: a source containing `struct tst_test` is a new-API
  test (parseable `file.c:line` subtest lines); anything else, shell
  scripts included, is judged by LTP's documented exit bits
  (`legacy-exit`).
- `group`: the source directory, the unit the per-syscall pass-rate
  breakdown aggregates over. It is an explicit field because the test
  id does not encode the directory reliably (`epoll01` builds from
  `epoll/`, `pipeio_1` from `../ipc/pipeio/`).

Resolution is convention plus a hand-maintained escape hatch: the exact
`<binary>.c` anywhere under `testcases/`, else the base source of a
`_64`/`_16` build variant in the same directory, else an explicit entry
in `tests/ltp/sweep-overrides.json` (six at the pinned release: the
`test_ioctl` shell script, the two multi-source `nftw` binaries, and
the three `sethostname` tests compiled from the `setdomainname`
sources). Anything unresolvable, ambiguous, or containing shell
metacharacters is a generation error, never a silent default, so a pin
bump that changes upstream's shapes fails loudly.

Two more generated properties keep the sweep honest:

- `sweep-slow`: kirk's `--exec-timeout` is one value per invocation,
  the maximum over the selection, so a single long-runner would make
  every hang in the bulk sweep cost its timeout. Entries whose override
  raises the timeout above the sweep default are promoted to their own
  tier and run separately.
- `unbuilt`: entries whose binary the cross build cannot produce (a
  missing optional library on the build host) are marked with a reason,
  excluded from selection, and announced with a count on every run;
  targeting one with `LTP_TEST` is an explicit error. The committed
  file carries these marks, and both `gen-sweep --check` and the
  fixture builder fail when the file drifts from a regeneration.
...
The committed baselines are snapshots of what each backend does today
at the pinned LTP/kirk versions, split by tier class so the curated
files stay hand-reviewable: `baseline-{elfuse,qemu}.json` cover the
curated tiers and `baseline-{elfuse,qemu}-sweep.json` cover the sweep
tiers (generated-scale files, reviewed through the record summary and
the pass-rate artifacts rather than line by line). The gate partitions
a selection by tier class, loads exactly the files that selection
needs, and merges them (an id recorded in both files is an error)
before one compare. The gate fails only on movement; "many tests are
BROKEN because elfuse lacks syscalls" is a green state once recorded,
which is what makes per-subtest on/off control work without patching
LTP.
...
## The reference annotates, it does not gate

The QEMU VM is the ground truth for what each test should do, but it is
not a gate on running elfuse: at sweep breadth the reference itself
legitimately fails tests (root-only behavior, kernel features the VM
lacks), and those tests must still run through elfuse and be baselined.
In `--backend all` mode QEMU runs first so its fresh results can
annotate the elfuse report; an elfuse-only run falls back to the
committed qemu baselines, and a test the reference has never recorded
degrades to `UNRECORDED` rather than an error.

The annotation is the per-test `reference` map in `gate-elfuse.json`:
`{"status": <qemu status>, "attesting": <bool>}`. Only a test the
reference passes attests Linux conformance when elfuse passes it too;
any other reference status leaves the elfuse result meaningful for the
movement gate but unattested. Each run prints how many selected tests
do not attest.

## Pass rates

Every run aggregates its digested statuses (`ltp_harness/passrate.py`)
into three rates, printed as a summary line and written to
`passrate-<backend>.json` plus a markdown table:

- **pass rate**: PASS over the whole selection, the headline number;
- **pass rate excluding skips**: PASS over the tests that actually ran,
  since a TCONF-heavy selection would otherwise flatter nobody;
- **conformance rate** (elfuse only): PASS on elfuse among the tests
  the reference passes, the fraction of attestable Linux behavior
  elfuse reproduces.

The markdown table (`passrate-<backend>.md`) groups by the manifest's
`group` field and sorts worst-first, so the weakest syscall families
surface immediately; each row carries the failing ids (capped at ten
with an explicit `+N more` marker; the JSON always holds the full
list). This is the artifact that prioritizes which subsystem to fix
next.
...
- `gate-<backend>.json`: machine-readable gate outcome (regressions,
  improvements, infos), the digested per-test observations, the
  `reference` attestation map (elfuse only), and the `passrate` block;
- `passrate-<backend>.json` / `passrate-<backend>.md`: the pass-rate
  aggregation and its worst-first per-group table;
...
3. builds the payload for the requested flavor. The default sweep
   flavor builds `lib/`, the whole `testcases/kernel/syscalls` tree,
   and the out-of-tree directories the sweep references, then stages
   through LTP's own per-directory `make install DESTDIR=...` rules so
   `INSTALL_TARGETS` scripts, helper binaries, and data files land in
   `testcases/bin` exactly as upstream installs them; it then
   regenerates the sweep manifest in memory and refuses to stage when
   the committed file drifts. The `--no-sweep` flavor (`LTP_SWEEP=0`)
   builds only the source directories of the curated tests (each id
   located by its `<id>.c` under `testcases/kernel/syscalls/`);
4. stages the rootfs: the installed test payload in
   `/opt/ltp/testcases/bin` (every ELF verified AArch64 via cross
   `readelf -h`; installed shell scripts are identified by magic and
   exempt), LTP's `COPYING` and a `Version` provenance file, the
   ...
   `/opt/elfuse-ltp/bin`, and one generated runtest file per populated
   tier (`/opt/ltp/runtest/elfuse-*`) rendered from the merged
   manifests;
...
6. writes `inventory.txt` (sorted sha256/mode/symlink line per rootfs
   ...
   busybox provenance and digest, helper digests, the built flavor),
   and `.complete` containing the input fingerprint, a sha256 over the
   flavor, `pin.json`, `manifest.json`, `manifest-sweep.json`,
   `sweep-overrides.json`, both helper sources, `sweep.py`, and
   `fixture.py` itself, so any input change invalidates the fixture.
...
Add a curated test: inspect the pinned LTP source, add the entry to
`manifest.json` (id, installed command, tier, group, timeout, result
format, helpers, data files), run `make gen-ltp-sweep` (the id leaves
the sweep manifest), `make build-ltp-fixture`, run both backends, then
`make record-ltp-baseline` and review the baseline diff in the same
commit as the manifest change. Sweep entries are never edited by hand:
their knobs live in `sweep-overrides.json` (raise a timeout, point at
an unresolvable source), followed by the same regenerate-and-record
workflow.
...
Bump a pin: edit `pin.json`, `make build-ltp-fixture FORCE=1` (which
stops at the sweep drift check), `make gen-ltp-sweep` and review the
selection diff, rebuild, run both backends, `make record-ltp-baseline`,
review the (possibly large, file:line shifted) baseline diff, and
commit pin, sweep manifest, and baselines together. The gate's pin
check makes skipping this workflow impossible.
