...
class ReportTest(TempDirTest):
    ...
    def test_markdown(self):
        ...
        text, red = report.markdown(self.dir)
        self.assertIn("| fake/host | pr | green | 1 | 0 | 0 | 0 |", text)
        self.assertEqual(text.count("| error |"), 2)
        self.assertTrue(red)  # a lane that will not load is not green
        text, red = report.markdown(self.dir / "none")
        self.assertIn("no conformance results", text)
        self.assertFalse(red)

    def test_markdown_reports_a_red_lane(self):
        report.write(self.dir / "fake" / "host" / "1", self.meta,
                     [case("fake:b/p", Status.FAIL, Verdict.UNEXPECTED_FAILURE)])
        text, red = report.markdown(self.dir)
        self.assertIn("| fake/host | pr | red | 0 | 0 | 0 | 1 |", text)
        self.assertTrue(red)
