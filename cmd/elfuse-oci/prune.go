	...
	for _, e := range entries {
		...
		if e.Name() == "sha256" {
			...
			for _, child := range children {
				if !child.IsDir() {
					// A sibling <hex>.lock whose cache dir is absent is an
					// orphan (every run creates the lock without ever
					// creating the plain dir; a failed unpack leaves one
					// too); sweep it, held locks excepted. A lock whose dir
					// exists belongs to that dir's own sweep below.
					hex, isLock := strings.CutSuffix(child.Name(), ".lock")
					if !isLock || opts.dryRun {
						continue
					}
					if _, err := os.Lstat(filepath.Join(top, hex)); !os.IsNotExist(err) {
						continue
					}
					if err := sweepOrphanRootfsLock(filepath.Join(top, hex)); err != nil {
						return rep, err
					}
					continue
				}
