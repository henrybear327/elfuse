...
teardown_live_run_lock() {
    ...
    # SIGKILL, deliberately: the claim above is that the kernel (not any
    # elfuse cleanup code) drops the flock, and a plain SIGTERM would let a
    # graceful teardown pass this test while the no-cleanup path regressed.
    kill -9 "$guest" 2>/dev/null || true
    wait "$guest" || true
    guest=""
