...
// storeRootfsPublished reports whether a store-managed rootfs cache path is
// already published, and refuses anything there that is not a real directory.
//
// The store names these paths by digest and publishes them only by renaming a
// staging directory into place, so a symlink or file found there did not come
// from this package. It must be refused rather than used: os.OpenRoot follows
// symlinks in the directory name it opens, and an existence probe that follows
// them reports "already unpacked", so a symlink at the digest path would
// redirect both the extraction and the guest that runs against the result out
// of the store. An explicit --rootfs is the user's own directory and keeps
// following links, which is why this guards the store-managed callers instead
// of unpackImage itself.
...
func applyLayer(root *os.Root, layer v1.Layer) error {
	...
	// Tar's end-of-archive marker (two zero blocks) arrives before the
	// compressed stream's end, so the decompressor never reads the gzip
	// trailer during the entry loop: drain it so CRC/ISIZE corruption fails
	// the unpack instead of vanishing in the deferred Close, whose error is
	// discarded.
	if _, err := io.Copy(io.Discard, r); err != nil {
		return fmt.Errorf("layer trailer: %w", err)
	}
	return nil
}
...
func applyEntry(root *os.Root, hdr *tar.Header, r io.Reader, applied map[string]bool) error {
	...
	if trimmed, ok := strings.CutPrefix(base, whiteoutPrefix); ok {
		// A whiteout names one sibling to remove, so the suffix must be a
		// plain file name. An empty (".wh."), dot (".wh.."), or dot-dot
		// (".wh...") suffix makes Join resolve to the containing directory or
		// above it, turning a one-entry removal into a subtree wipe; a
		// separator would escape the marker's own directory. A malformed
		// layer must fail rather than delete more than it named.
		...
		// A lower layer may have planted a symlink at a parent component;
		// RemoveAll resolves in-root links, so removing through one deletes
		// under the link target, files the layer never named (ensureParent's
		// rule applied to removal, except a whiteout must not materialize
		// directories either). A parent that is absent or not a real
		// directory means the literal path does not exist: the whiteout is a
		// no-op, and any replacement of the parent is the job of the layer's
		// own directory entry.
		if dir := filepath.Dir(name); dir != "." {
			ok, err := realDirChain(root, dir)
			if err != nil || !ok {
				return err
			}
		}
...
// realDirChain reports whether every component of dir exists as a real
// directory (no symlink, no plain file) inside root. Callers that must not
// create or replace components (the whiteout path) use this where entry
// creation would use ensureParent.
func realDirChain(root *os.Root, dir string) (bool, error) {
	cur := ""
