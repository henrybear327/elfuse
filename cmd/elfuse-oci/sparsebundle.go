...
func provisionCaseSensitive(bundleDir, mountPath, size string) (*csMount, error) {
	...
	if _, err := os.Stat(image); os.IsNotExist(err) {
		if err := runHdiutil("create", image,
			"-fs", "Case-sensitive APFS",
			"-size", size,
			"-type", "SPARSEBUNDLE",
			"-volname", "elfuse_sysroot"); err != nil {
			return fail(err)
		}
	} else if err != nil {
		return fail(err)
...
// runHdiutil runs `hdiutil <verb> <flags...> <path>` and folds the output into
// the error, since hdiutil reports the actual cause (a busy volume, a bad
// size) only in its output. path is both the operand and the subject of the
// error, so the two cannot disagree. The attach path does not use this: it
// parses a plist from stdout and so must keep stderr separate.
func runHdiutil(verb, path string, flags ...string) error {
	argv := append([]string{verb}, flags...)
	argv = append(argv, path)
	out, err := exec.Command("hdiutil", argv...).CombinedOutput()
	if err != nil {
		return fmt.Errorf("hdiutil %s %s: %w: %s", verb, path, err, out)
	}
	return nil
}

var detachForce = func(mountPath string) error {
	return runHdiutil("detach", mountPath, "-force")
}
