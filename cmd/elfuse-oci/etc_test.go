...
// TestWriteRuntimeFileLeavesNoTempOnRenameFailure drives the staging-cleanup
// branch the read-only-dir case cannot reach (there the create fails before
// any temp exists): a non-empty directory at the destination name makes the
// final rename fail after the temp file was written, so the cleanup must
// remove it. Regression guard: the cleanup is already correct; deleting the
// rename branch's root.Remove(tmp) turns this red.
func TestWriteRuntimeFileLeavesNoTempOnRenameFailure(t *testing.T) {
	dir := t.TempDir()
	root, err := os.OpenRoot(dir)
	if err != nil {
		t.Fatal(err)
	...
	if err := writeRuntimeFile(root, "resolv.conf", []byte("nameserver 8.8.8.8\n")); err == nil {
		t.Fatal("writeRuntimeFile onto a non-empty directory succeeded, want error")
	}
	entries, err := os.ReadDir(dir)
	if err != nil {
		t.Fatal(err)
