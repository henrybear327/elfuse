...
func (p *layerPolicy) filter(hdr *tar.Header) error {
	...
	record := layerRecord{kind: hdr.Typeflag, layer: p.layer}
...
func (p *layerPolicy) resolveDirectory(dir string) (string, error) {
	...
	for len(pending) != 0 {
		...
		case "..":
			if len(resolved) == 0 {
				return "", fmt.Errorf("parent path %q escapes the rootfs", dir)
			}
...
// Read must not prepare another entry while returning the current entry's data.
...
func (r *filteredLayer) Read(dst []byte) (int, error) {
	...
	if r.body != 0 {
		n, err := r.tr.Read(r.buf[:min(int64(len(r.buf)), r.body)])
		if n != 0 {
			_, r.err = r.tw.Write(r.buf[:n])
			r.body -= int64(n)
		}
		if err != nil && (err != io.EOF || r.body != 0) {
