...
Turns raw GoogleTest output into a verdict the lane can gate on. Every
discovered case is matched against the expectations manifest, which must
cover the discovered set exactly: a row naming no real case and a case with
no row are both errors, so an allowlist cannot quietly outlive the test it
names. Anything that is not a clean reported result (unparseable or
truncated XML, a suite whose exit code disagrees with its XML, a case the
suite never reached) is BROKEN and fatal rather than absent, because the
one outcome this file exists to prevent is a run that failed to happen
reading as a run that passed.
"""
