...
// Ownership is never applied, so a setuid bit would name the invoking user on
// every host, not only on macOS.
...
// Only a rewritten header needs its format reset. Resetting every header would
// round a PAX timestamp to whole seconds.
...
func gzipLayer(t *testing.T, raw []byte) []byte {
	t.Helper()
	var b bytes.Buffer
	zw := gzip.NewWriter(&b)
...
func TestApplyLayerReportsMidStreamFailure(t *testing.T) {
	entries := []tarEntry{{Name: "bad", Link: "missing", Type: tar.TypeLink}}
	for i := range 200 {
		entries = append(entries, tarEntry{Name: fmt.Sprintf("f%03d", i), Body: strings.Repeat("x", 64*1024)})
	}
	raw := gzipLayer(t, buildLayerTar(t, entries))
	dest := t.TempDir()
	err := applyLayer(dest, bytes.NewReader(raw), testLayerPolicy(t, dest), unpackOptions())
	if err == nil {
...
func TestApplyLayerStopsOnCancellationMidLayer(t *testing.T) {
	...
	dest := t.TempDir()
	err := applyLayer(dest, contextReader{ctx: ctx, r: src}, testLayerPolicy(t, dest), unpackOptions())
	if err == nil {
