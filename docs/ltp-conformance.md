...
on a case-insensitive sysroot elfuse stores a newly created guest file
under its escaped spelling, which the host cannot resolve by the guest
pathname, and the exit code already travels launcher to elfuse to kirk. When elfuse itself dies
